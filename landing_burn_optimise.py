...
class LandingBurn:
    def __init__(self, individual=None):
        ...
        self.max_gimbal_angle_rad = math.radians(20)
...
def objective_func(individual):
    ...
    Evaluates controller performance with given parameters.
    
    individual: array of 4 parameters
    [Kp_pitch_rate, Kd_pitch_rate, Kp_pitch, Kd_pitch]
    
    returns: cost to minimize (negative reward)
    """
    # Use the individual parameters directly - all 4 of them
    landing_burn = LandingBurn(individual)
    landing_burn.run_closed_loop()
    obj = -landing_burn.performance_metrics()  # Negate reward to get cost for minimization
    print('Objective: ', obj)
    return obj

def save_gains(xopt):
    """Save the optimized gains to a CSV file"""
    xopt = np.array(xopt)
    print(f"Best parameters found: {xopt}")
    
    # Fixed values for N parameters
    N_pitch_rate = 5
    N_pitch = 5
    ...
    # Save to file
    os.makedirs('data/reference_trajectory/landing_burn_optimal', exist_ok=True)
    gains.to_csv('data/reference_trajectory/landing_burn_optimal/controller_gains.csv', index=False)

def tune_landing_burn():
    """
    Use PSO to find optimal controller parameters
    """
    ...
    # Save the optimized gains
    save_gains(xopt)
    
    # Run a final simulation with the best parameters (just the 4 parameters)
