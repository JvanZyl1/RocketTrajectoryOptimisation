...
def gru_critic_loss(model, batch, target_model, gamma=0.99):
    losses = []
    for obs_seq, act, rew, next_obs, done in batch:
        obs_seq = obs_seq.unsqueeze(0)
        with torch.no_grad():
            next_input = torch.cat([obs_seq[0, 1:], next_obs.unsqueeze(0)], dim=0).unsqueeze(0)
            target_q = rew + gamma * (1 - done) * target_model(next_input, act.unsqueeze(0))
        pred_q = model(obs_seq, act.unsqueeze(0))
...
def main():
    ...
    batch = [(obs.to(DEVICE), act.to(DEVICE), rew.to(DEVICE), next_obs.to(DEVICE), done.to(DEVICE)) for obs, act, rew, next_obs, done in batch]

    loss_double = double_critic_loss(double_critic, batch, target_double_critic)
    loss_gru = gru_critic_loss(gru_critic, batch, target_gru_critic)

    print(f"Double Critic Loss: {loss_double.item():.6f}")
    print(f"GRU Critic Loss:    {loss_gru.item():.6f}")
