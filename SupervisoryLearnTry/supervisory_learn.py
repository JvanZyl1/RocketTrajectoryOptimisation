...
def train_network(inputs, targets, epochs=25000, learning_rate=0.001):
...
def load_data_from_csv(file_path):
    ...
    normalised_targets = np.array([target/np.max(target) for target in targets])
...
def save_model(state, filename='supervisory_network.pkl'):
    with open(filename, 'wb') as f:
        pickle.dump(state.params, f)
    print(f'Model parameters saved to {filename}')

# Function to load the model parameters
def load_model(filename='supervisory_network.pkl'):
...
if __name__ == "__main__":
    ...
    for idx in random_indices:
        mean, std = Actor(action_dim=targets.shape[1]).apply({'params': loaded_params}, inputs[idx].reshape(1, -1))
