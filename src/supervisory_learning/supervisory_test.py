...
def ballistic_arc_descent_supervisory_test(inputs, flight_phase, state_network, targets, hidden_dim, number_of_hidden_layers, reference_data):
    ...
    plt.xlabel('Time')
    plt.ylabel('u0')
    plt.title('u0 Over Time (RCS Throttle)')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'results/SupervisoryLearning/{flight_phase}/u0Imitation.png')
...
def re_entry_burn_supervisory_test(inputs, flight_phase, state_network, targets, hidden_dim, number_of_hidden_layers, reference_data):
    ...
    plt.xlabel('Time')  
    plt.ylabel('u1')
    plt.title('u1 Over Time (RCS Throttle)')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'results/SupervisoryLearning/{flight_phase}/u1Imitation.png')
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(reference_data['u2'][1:-2], label='PID')
    plt.plot(u2_learnt[1:-2], linestyle='--', label='Imitation')
    plt.xlabel('Time')  
    plt.ylabel('u2')
    plt.title('u2 Over Time (RCS Throttle)')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'results/SupervisoryLearning/{flight_phase}/u2Imitation.png')
    plt.close()
