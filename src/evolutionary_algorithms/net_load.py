...
def load_pso_weights():
    ...
    #print(f"Found {len(weight_columns)} weight columns and {len(bias_columns)} bias columns")
    ...
    #print(f"Detected layer indices: {all_layer_indices}")
    ...
    if 0 in layer_weights and layer_weights[0]:
        ...
        if 0 in layer_biases and layer_biases[0]:
            ...
            for idx in all_layer_indices[1:]:
                ...
                hidden_dim = 10
                #print(f"Warning: Could not determine hidden_dim, using default: {hidden_dim}")
        ...
        input_dim = 5
        hidden_dim = 10
        #print(f"Warning: Could not determine input_dim, using default: {input_dim}")
    ...
    if output_layer_idx in layer_biases and layer_biases[output_layer_idx]:
        ...
        # Default if we can't determine
        output_dim = 2
        #print(f"Warning: Could not determine output_dim, using default: {output_dim}")
    
    #print(f"Network architecture: input_dim={input_dim}, hidden_dim={hidden_dim}, output_dim={output_dim}")
    ...
    for i, layer_idx in enumerate(all_layer_indices):
        ...
        if layer_idx in layer_biases:
            ...
            if i == 0:
                ...
                layer_bias_values = [0.0] * hidden_dim
            #print(f"Warning: No biases found for layer {layer_idx}, using zeros")
        ...
        #print(f"Added layer {i} (index {layer_idx}): weights shape {weights.shape}, biases shape {np.array(layer_bias_values).shape}")
