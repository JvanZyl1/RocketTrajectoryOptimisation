...
def universal_physics_plotter(env,
    ...
    assert env.flight_phase in ['subsonic', 'supersonic', 'flip_over_boostbackburn', 'ballistic_arc_descent', 'landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']
    x_array = []
    y_array = []
    vx_array = []
    vy_array = []
    ...
    while not done_or_truncated:
        ...
        if env.flight_phase in ['subsonic', 'supersonic']:
            ...
            acs_F_perpendicular_right.append(info['action_info']['acs_info']['F_perpendicular_R'])
            acs_Moment.append(info['action_info']['acs_info']['Mz'])
        elif env.flight_phase == 'landing_burn_pure_throttle':
            throttle.append(info['action_info']['throttle'])
            acs_alpha_local_left_rad.append(info['action_info']['acs_info']['alpha_local_left_rad'])
            acs_alpha_local_right_rad.append(info['action_info']['acs_info']['alpha_local_right_rad'])
    ...
    if type == 'pso' or type == 'rl':
        ...
        if env.flight_phase in ['subsonic', 'supersonic']:
            ...
            if truncation_id == 0:
                ...
                print(f'Truncated as unknown reason; truncation_id: {truncation_id}')
        elif env.flight_phase in ['landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']:
            if truncation_id == 0:
                print(f'It is done, Jonny go have a cerveza.')
            elif truncation_id == 1:
    ...
    if len(time) > 0:
        ...
        if env.flight_phase in ['subsonic', 'supersonic', 'landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']:  
        ...
        elif env.flight_phase in ['ballistic_arc_descent', 'landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']:
        ...
        if not type == 'physics':
            ...
            if env.flight_phase == 'subsonic':
                ...
                ax1.set_ylabel('Horizontal position [m]', fontsize=20)
            elif env.flight_phase in ['supersonic', 'flip_over_boostbackburn', 'ballistic_arc_descent', 'landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']:
            ...
            if env.flight_phase in ['ballistic_arc_descent', 'landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']:
            ...
            elif flight_phase in ['ballistic_arc_descent', 'landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']:
            ...
            if env.flight_phase != 'ballistic_arc_descent':
                ...
                if env.flight_phase in ['subsonic', 'supersonic', 'ballistic_arc_descent', 'landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']:
                ...
                if env.flight_phase == 'subsonic':
                    ...
                    ax6.legend(fontsize=20, loc='lower right')
                elif env.flight_phase == 'landing_burn' or env.flight_phase == 'landing_burn_ACS' or env.flight_phase == 'landing_burn_pure_throttle':
                    ax6.legend(fontsize=20, loc='lower right')
                ax6.tick_params(axis='both', which='major', labelsize=18)
                ax6.grid(True)
        ...
        if flight_phase in ['landing_burn', 'landing_burn_ACS', 'landing_burn_pure_throttle']:
            plt.figure(figsize=(20, 15))
