...
class ReEntryBurnGainSchedule:
    def __init__(self, individual):
        self.dt = 0.1
        self.landing_burn_altitude = 3000
    ...
    def run_closed_loop(self):
        ...
        theta_dot_5000m = None
        alpha_effective_rad_5000m = None
        while y > self.landing_burn_altitude:
            self.closed_loop_step()
            x, y, vx, vy, theta, theta_dot, gamma, alpha, mass, mass_propellant, time = self.state
            alpha_effective_rad = gamma - theta - math.pi
            total_alpha_effective_rad += alpha_effective_rad
            max_alpha_effective_rad = max(max_alpha_effective_rad, abs(alpha_effective_rad))
            if y < 5000:
                if theta_dot_5000m is None: 
                    theta_dot_5000m = theta_dot
                    alpha_effective_rad_5000m = alpha_effective_rad
        return total_alpha_effective_rad, max_alpha_effective_rad, alpha_effective_rad, theta_dot,theta_dot_5000m, alpha_effective_rad_5000m
...
def objective_function(re_entry_burn_class, individual):
    ...
    total_alpha_effective_rad, max_alpha_effective_rad, _, _, _, _ = re_entry_burn_class.run_closed_loop()
...
def constraint_max_alpha_effective_rad(re_entry_burn_class, individual):
    re_entry_burn_class.reset(individual)
    total_alpha_effective_rad, max_alpha_effective_rad, _, _, _, _ = re_entry_burn_class.run_closed_loop()
    return 3.5 - math.degrees(max_alpha_effective_rad)

def constraint_final_alpha_effective_rad(re_entry_burn_class, individual):
    re_entry_burn_class.reset(individual)
    total_alpha_effective_rad, max_alpha_effective_rad, final_alpha_effective_rad, _, _, _ = re_entry_burn_class.run_closed_loop()
    return (0.1 - abs(math.degrees(final_alpha_effective_rad)))*25

def constraint_final_pitch_rate(re_entry_burn_class, individual):
    re_entry_burn_class.reset(individual)
    total_alpha_effective_rad, max_alpha_effective_rad, final_alpha_effective_rad, final_pitch_rate_rad, _, _ = re_entry_burn_class.run_closed_loop()
    return (0.005 - abs(math.degrees(final_pitch_rate_rad)))*100

def constraint_final_pitch_rate_5000m(re_entry_burn_class, individual):
    re_entry_burn_class.reset(individual)
    total_alpha_effective_rad, max_alpha_effective_rad, final_alpha_effective_rad, final_pitch_rate_rad, final_pitch_rate_5000m, _ = re_entry_burn_class.run_closed_loop()
    return (0.005 - abs(math.degrees(final_pitch_rate_5000m)))*100

def constraint_final_alpha_effective_rad_5000m(re_entry_burn_class, individual):
    re_entry_burn_class.reset(individual)
