...
n_e = 4                                  # Select number of landing burn engines
...
def objective(x, sim_results=None):
    ...
    smoothness_penalty = abs(np.sum(np.diff(u)**2))/x[-1]
    return (m0 - sim_results[2])/100000 + smoothness_penalty*10
...
class OptimizationCallback:
    ...
    def run_simulation(self, x):
        ...
        if tuple(x) not in self.simulation_cache:
            self.simulation_cache[tuple(x)] = simulate(u, T_burn)
...
def plot_simulation_results(u_opt, T_burn_opt):
    ys, vys, m_final, margins, mps, vy_dot  = simulate(u_opt, T_burn_opt)
...
u_opt = x_opt[:-1]
T_end_opt = x_opt[-1]
save_final_parameters(u_opt, T_end_opt,
